...
static bool channel_set_busy(struct scmi_server_channel *chan)
{
...
static void channel_release_busy(struct scmi_server_channel *chan)
{
	chan->busy = false;
}

static struct smt_header *channel_to_smt_hdr(struct scmi_server_channel *chan)
{
	return (struct smt_header *)chan->shm_addr;
}

static void set_status_flags(uint32_t *status, uint32_t flags)
{
	mmio_setbits_32((uintptr_t)status, flags);
}

/*
 * Creates a SCMI message instance in secure memory and push it in the SCMI
 * server drivers. Message structure contains SCMI protocol meta-data and
...
{
	struct scmi_server_channel *chan;
	...
	/* Update message length with the mength of the response message */
...
out:
	if (error) {
		VERBOSE("SCMI error");
		set_status_flags(&smt_hdr->status, SMT_STATUS_ERROR);
	}

	set_status_flags(&smt_hdr->status, SMT_STATUS_FREE);
}
...
{
	struct scmi_server_channel *chan = plat_scmi_get_channel(agent_id);
